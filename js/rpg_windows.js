...
Window_Base.prototype.processNormalCharacter = function(textState) {
    ...
    if (Window_Base.fixedWidth == 0)
        textState.x += w;
    else
        textState.x += Window_Base.fixedWidth;
};
...
Window_Options.prototype.addGeneralOptions = function() {
    ...
    this.addCommand("Graphic Content", 'graphicContent');
};
...
Window_SavefileList.prototype.drawContents = function(info, rect, valid) {
    ...
    if (y2 >= lineHeight) {
        this.drawPlaytime(info, rect.x - 16, y2, rect.width); // Too far right for some reason
    }
};
